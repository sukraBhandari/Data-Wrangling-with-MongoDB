...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            #count += 1
            for x in fields:
                if fields[x] in CLASSIFICATION: 
                    if line[x] == 'NULL':
                        line[x] = None
                    classify.update({fields[x] : line[x]})
                else:
                    if fields[x] == 'label':
                        d[fields[x]] = line[x].split(" ")[0]
                    elif line[x] == 'NULL':
                        d[fields[x]] = None
                    elif fields[x] == 'synonym' and line[x] is not None:
                        #print line[x]
                        d[fields[x]] = parse_array(line[x])
                    else:
                        d[fields[x]] = line[x]                 
            d.update({'classification' : classify}) 
            if d['name'] is None:
                d['name'] = d['label']
            data.append(d)
    #print data[0], count
