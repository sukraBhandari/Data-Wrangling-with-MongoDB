...
def article_overview(kind, period):
    ...
    for each in data:
        d = {}   
        d[each['section']] = each['title'].encode('utf-8')
        titles.append( d)
        urls.append([x['url'] for other in each['media'] for x in other['media-metadata']  if x['format'] == 'Standard Thumbnail'])

        #for other in each['media']:
            #for x in other['media-metadata']:
                #if x['format'] == 'Standard Thumbnail':
                    #urls.append( x['url']) 
    print(titles, list(itertools.chain(*urls)))
    return (titles, list(itertools.chain(*urls)))
