...
def add_field(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            binomial = line['binomialAuthority_label']
            if binomial != 'NULL':
                data[label] = binomial
...
def update_db(data, db):
    print (data)
    for each in data:
       query = db.arachnid.update({"label" : each},
                                  {"$set" : {"classification.binomialAuthority": data[each]}})
    #q = db.arachnid.find_one({"label" : "Zealanapis"})
    #pprint.pprint(q)
